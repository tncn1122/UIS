...
export default function ResendRegisterEmailModal({
  isShown,
  onClose,
  currentUserEmail,
  ModelService,
  ...
  const onOk = () => {
    if (usernameEmail) {
      ModelService.resendRegistrationEmail({ username_email: usernameEmail })
        .then(() => {
          setUsernameEmail('')
  ...
  return (
    <CustomModal
      title={<span className="typography-menu">{TEXT_UI_AUTH.REGISTER.RESEND_EMAIL}</span>}
    ...
    >
      <div className="mb-20">
        <span className="typography-menu-subtext">
          Làm theo các bước sau để nhận lại email kích hoạt
        </span>
        <div>
        ...
        <div>
          <span className="typography-menu-subtext">
            2. Chờ thông tin kích hoạt được gửi đến địa chỉ email đã đăng kí trên Code Tour
          </span>
        </div>
        <div>
