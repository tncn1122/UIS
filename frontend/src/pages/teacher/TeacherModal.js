...
const TeacherModal = (props) => {
  ...
  const onSubmit = value => {
    ...
    value.role = ENUM_ROLE[2] // teacher
    if (modalType === 'edit') {
      editTeacher(value)
    }
    else {
  ...
  return (
    <>
      <Modal
        visible={visible}
        title={modalType === 'edit' ? "Chỉnh sửa Giảng Viên" : "Tạo Giảng Viên mới"}
        okText="OK"
        width={1000}
        cancelText="Hủy"
        onCancel={() => {
        ...
        forceRender
      >
        <Form form={form} layout="vertical" name="teacher-form" onValuesChange={(_, values) => setFormValues(values)}>
          <Row gutter={[40, 10]}>
            <Col span={12}>
              <Form.Item
                label="Mã Giảng Viên"
            ...
            <Col span={12}>
              <Row gutter={[10, 20]}>
                ...
                <Col span={3}>
                  <Tooltip placement="bottom" title={'Sinh mật khẩu dựa trên mã giảng viên.'}>
                    <Button
                      style={{
                        marginTop: '30px',
                        padding: '0px 0px',
